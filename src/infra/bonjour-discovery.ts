...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    const line = raw.trim();
    if (!line || !line.includes("_openclaw-gw._tcp")) continue;
...
async function discoverViaDnsSd(
  ...
  const browse = await run(["dns-sd", "-B", "_openclaw-gw._tcp", domain], {
    timeoutMs,
  });
  const instances = parseDnsSdBrowse(browse.stdout);
  const results: GatewayBonjourBeacon[] = [];
  for (const instance of instances) {
    const resolved = await run(["dns-sd", "-L", instance, "_openclaw-gw._tcp", domain], {
      timeoutMs,
    });
...
async function discoverWideAreaViaTailnetDns(
  ...
  const probeName = `_openclaw-gw._tcp.${domain.replace(/\.$/, "")}`;
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (!line) continue;
    if (line.startsWith("=") && line.includes("_openclaw-gw._tcp")) {
      if (current) results.push(current);
      const marker = " _openclaw-gw._tcp";
...
async function discoverViaAvahi(
  ...
  const args = ["avahi-browse", "-rt", "_openclaw-gw._tcp"];
