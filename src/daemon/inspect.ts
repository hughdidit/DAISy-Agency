...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "openclaw" | "openclaw" | "openclaw";
  legacy?: boolean;
};
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.openclaw.") || label.startsWith("com.openclaw.");
}
...
function isIgnoredLaunchdLabel(label: string): boolean {
  return (
    ...
    resolveLegacyGatewayLaunchAgentLabels(process.env.OPENCLAW_PROFILE).includes(label)
  );
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("openclaw") || lower.includes("openclaw");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "openclaw" : "openclaw",
        legacy: true,
      });
      continue;
    }
