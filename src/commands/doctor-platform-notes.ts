...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".openclaw", "disable-launchagent");
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("OPENCLAW_GATEWAY_TOKEN");
  const envPassword = await getenv("OPENCLAW_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `OPENCLAW_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `OPENCLAW_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
