...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("sanitizes sandbox docker crashes before replying", async () => {
    await withTempStateDir(async (stateDir) => {
      ...
      expect(res).toMatchObject({
        text: "⚠️ Agent failed before reply: Sandbox runtime capability failure during sandbox startup (sandbox image inspection): Docker CLI could not inspect the sandbox image in the gateway runtime. Fix: Check Docker CLI/socket access and provision the configured sandbox image/profile. See: openclaw doctor sandbox. Cause: low-level runtime crash details redacted.\nLogs: openclaw logs --follow",
      });
