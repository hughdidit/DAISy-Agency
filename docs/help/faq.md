...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [I can't access docs.molt.bot (SSL error). What now?](#i-cant-access-docsopenclaw-ssl-error-what-now)
...
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  ...
  - [Can multiple people use one WhatsApp number with different OpenClaws?](#can-multiple-people-use-one-whatsapp-number-with-different-openclaws)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two OpenClaws talk to each other (local + VPS)?](#can-two-openclaws-talk-to-each-other-local-vps)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of OpenClaws one CEO and many agents](#is-there-a-way-to-make-a-team-of-openclaws-one-ceo-and-many-agents)
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/moltai/openclawbot-*.log | head -1)"
   ```
...
https://github.com/moltai/openclawbot/issues
https://github.com/moltai/openclawbot/pulls
...
https://github.com/moltai/openclawbot/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docsopenclaw SSL error What now
...
detail: [Troubleshooting](/help/troubleshooting#docsopenclaw-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: https://spa.xfinity.com/check_url_status.

If you still can't reach the site, the docs are mirrored on GitHub:
https://github.com/moltai/openclawbot/tree/main/docs
...
### Can multiple people use one WhatsApp number with different OpenClaws
...
Use managed overrides instead of editing the repo copy. Put your changes in `~/.openclaw/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.clawdai/openclawbot.json`). Precedence is `<workspace>/skills` > `~/.openclaw/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.clawdai/openclawbot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.openclaw/skills` → bundled → `skills.load.extraDirs`. `clawdhub` installs into `./skills` by default, which OpenClaw treats as `<workspace>/skills`.
...
OpenClaw reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.clawdai/openclawbot.json`):
...
### Can two OpenClaws talk to each other local VPS
...
Recover:
- Restore from backup (git or a copied `~/.clawdai/openclawbot.json`).
...
### Is there a way to make a team of OpenClaws one CEO and many agents
...
- edit `agents.defaults.model` in `~/.clawdai/openclawbot.json`
...
```
/tmp/moltai/openclawbot-YYYY-MM-DD.log
```
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [OpenClaw assistant setup](/start/clawd) and [Agent send](/tools/agent-send).
...
Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/moltai/openclawbot/discussions).
