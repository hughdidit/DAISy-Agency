...
OpenClaw uses a dedicated workspace directory for the agent. Default: `~/clawd` (configurable via `agents.defaults.workspace`).
...
- For browser-driven verification, use `openclaw browser` (tabs/status/screenshot) with the clawd-managed Chrome profile.
