...
- Set `gateway.bind: "tailnet"` in `~/.clawdai/openclawbot.json` on the gateway host.
...
2) Configure Tailscale split DNS for `openclaw.internal` pointing at that DNS server.
