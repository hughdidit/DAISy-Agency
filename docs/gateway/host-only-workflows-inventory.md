...
| `HWI-007` | Runtime debug overrides                    | `/debug` show, set, unset, and reset actions                                                        | `docs/tools/slash-commands.md`; `src/auto-reply/commands-registry.data.ts`                                                                               | `likely-convertible`      | Debug overrides mutate Gateway runtime state in memory and are explicitly owner-only and disabled by default.                            | Environmental | Chat commands + config | Keep disabled by default; use only for operator diagnostics, not agent task execution.                        | SBX-504 outcome: retained as operator-only break-glass control-plane flow (`runtime-debug`) with replies, logs, and audit findings.                               |
| `HWI-008` | Gateway restart command                    | `/restart`                                                                                          | `docs/tools/slash-commands.md`; `src/auto-reply/commands-registry.data.ts`                                                                               | `clearly-host-only-today` | Restart controls the Gateway host process rather than a sandboxed session.                                                               | Environmental | Chat commands + ops    | Keep gated and documented as operational control, separate from sandbox readiness.                            | SBX-504 outcome: retained as operator break-glass gateway control (`gateway-restart`) with replies, logs, and audit findings.                                     |
