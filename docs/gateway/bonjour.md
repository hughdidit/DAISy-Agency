...
3) Configure Tailscale **split DNS** so `openclaw.internal` resolves via that
   DNS server for clients (including iOS).

OpenClaw standardizes on `openclaw.internal.` for this mode. iOS/Android nodes
browse both `local.` and `openclaw.internal.` automatically.
...
```json5
{
  ...
  discovery: { wideArea: { enabled: true } } // enables openclaw.internal DNS-SD publishing
}
```
...
- serve `openclaw.internal.` from `~/.openclaw/dns/openclaw.internal.db`
...
`_openclaw-gw._tcp` in `openclaw.internal.` without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.clawdai/openclawbot.json`.
...
- `gateway.bind` in `~/.clawdai/openclawbot.json` controls the Gateway bind mode.
