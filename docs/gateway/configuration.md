---
summary: "All configuration options for ~/.clawdai/openclawbot.json with examples"
read_when:
...
OpenClaw reads an optional **JSON5** config from `~/.clawdai/openclawbot.json` (comments + trailing commas allowed).

If the file is missing, OpenClaw uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
use `config.patch` or `openclaw config set`. Keep a backup of `~/.clawdai/openclawbot.json`.
...
- Default log file: `/tmp/moltai/openclawbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/moltai/openclawbot.log`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "openclaw", "clawd"] } }
    ]
  }
}
```
...
OpenClaw can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
macOS app behavior:
- OpenClaw.app watches `~/.clawdai/openclawbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawdai/openclawbot.json` (or `OPENCLAW_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.clawdai/openclawbot.json` (or `OPENCLAW_CONFIG_PATH`)
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__openclaw/ws`
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openclaw-bridge._tcp` under `~/.openclaw/dns/` using the standard discovery domain `openclaw.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `openclaw.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `openclaw.internal` via that server
