...
## Quick check: `openclaw security audit` (formerly `openclaw security audit`)
...
# (On older installs, the command is `openclaw ...`.)
```
...
- `~/.moltai/openclawbot.json`: `600` (user read/write only)
...
1. Check Gateway logs: `/tmp/moltai/openclawbot-YYYY-MM-DD.log` (or `logging.file`).
