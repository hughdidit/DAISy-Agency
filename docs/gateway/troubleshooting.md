...
- File logs (always): `/tmp/moltai/openclawbot-YYYY-MM-DD.log` (or your configured `logging.file`)
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${OPENCLAW_CONFIG_PATH:-$HOME/.clawdai/openclawbot.json}"
```
...
tail -f "$(ls -t /tmp/moltai/openclawbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/moltai/openclawbot-*.log | head -1)" | tail -20
```
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/moltai/openclawbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
#   ${OPENCLAW_CONFIG_PATH:-$HOME/.clawdai/openclawbot.json} -> { logging: { level: "trace" } }
#
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/moltai/openclawbot-YYYY-MM-DD.log` (or `logging.file`) |
...
tail -20 /tmp/moltai/openclawbot-*.log
```
