...
Card mutation RPCs require `expectedVersion` for optimistic concurrency. A stale version returns a
retryable invalid-request error so clients can reload and retry intentionally.
