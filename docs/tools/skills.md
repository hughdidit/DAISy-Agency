...
`<sandbox workdir>/.openclaw-readonly/` (usually `/workspace/.openclaw-readonly/`)
when present. If those paths are still missing, it fails closed with a setup
error instead of trying a host-side fallback.
