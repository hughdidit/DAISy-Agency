...
# Container Access
clawdock-shell() {
  _clawdock_compose exec openclaw-gateway bash
}
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" openclaw-gateway \
    bash -c 'openclaw config set gateway.remote.token "$TOKEN" && openclaw config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec openclaw-gateway \
    bash -c "openclaw config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
