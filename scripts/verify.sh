...
if [[ -n "${GCE_INSTANCE_NAME:-}" ]]; then
  ...
  mcp_smoke_output="$(
    gce_ssh "sudo docker exec ${container_escaped} node -e 'const fs=require(\"fs\"); const path=require(\"path\"); const { spawnSync } = require(\"child_process\"); const entryPath=require.resolve(\"mongodb-mcp-server\"); const packageRoot=path.resolve(path.dirname(entryPath), \"..\", \"..\"); const packageJson=JSON.parse(fs.readFileSync(path.join(packageRoot, \"package.json\"), \"utf8\")); const binPath=path.join(packageRoot, packageJson.bin[\"mongodb-mcp-server\"]); console.log(\"node_version=\" + process.version); console.log(\"mongodb_mcp_bin=\" + binPath); const result=spawnSync(process.execPath, [binPath, \"--version\"], { encoding: \"utf8\" }); if ((result.stdout || \"\").trim()) console.log(\"mongodb_mcp_version=\" + result.stdout.trim()); if (result.status !== 0) { if (result.stderr) process.stderr.write(result.stderr); process.exit(result.status ?? 1); }'"
