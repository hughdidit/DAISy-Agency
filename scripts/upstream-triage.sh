...
APPLY="false"
OPEN_PR="false"
RUN_TS="$(date +%Y-%m-%d-%H%M)"  # single timestamp for the entire run
...
Output:
  docs/upstream-candidates/YYYY-MM-DD-HHMM.md   Triage report (report mode only)
EOF
  exit 0
}
...
# =============================================================================
# Generate markdown report (report mode only — apply mode skips this)
# =============================================================================

if [[ "${APPLY}" != "true" ]]; then
  mkdir -p "${REPORT_DIR}"
  REPORT_FILE="${REPORT_DIR}/${RUN_TS}.md"

  log "Writing report to ${REPORT_FILE}..."

  {
    echo "# Upstream Triage Report — ${RUN_TS}"
    echo ""
    ...
    echo "---"
    echo "*Generated by \`scripts/upstream-triage.sh\` on ${RUN_TS} — no upstream code was executed*"
  } > "${REPORT_FILE}"
...
if [[ "${APPLY}" == "true" ]]; then
  ...
  # Conflicts are committed with markers intact — the PR reviewer resolves them.
...
"
    ...
    pr_title="cherry-pick: upstream ${cat} commits (${RUN_TS})"
...
Additionally, this PR may show merge conflicts against \`${BASE_REF}\` due to
...
---
*Generated by \`scripts/upstream-triage.sh --apply --open-pr\` on ${RUN_TS} — no upstream code was executed*
EOPR
)"
