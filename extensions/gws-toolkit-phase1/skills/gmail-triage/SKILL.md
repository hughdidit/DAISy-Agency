...
7. When a message is fully handled by a direct reply or a no-reply decision, mark the email as read if the available Gmail tool and route policy support that mutation. Keep messages unread when a draft, escalation, or pending human approval remains. If no mark-read or label mutation is available, report that tool gap instead of claiming the email was marked read.
