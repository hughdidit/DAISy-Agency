...
import { runToolkitCommand } from "../../src/commands/helpers.js";
...
describe("integration: delegated direct Google API transport", () => {
  ...
  it("evaluates direct transport postPolicy against transformed payloads", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "gws-direct-post-policy-"));
    const credentialsFile = path.join(root, "service-account.json");
    await fs.writeFile(
      credentialsFile,
      JSON.stringify({
    ...
    const result = await runToolkitCommand({
      ctx: {
        agentId: "main",
        sessionKey: "agent:main:main",
        googleWorkspaceEmail: "daisy.ai@hughdidit.com",
      ...
      service: "calendar",
      action: "list_events",
      payload: { action: "list_events", calendarId: "primary" },
      readOnly: true,
      buildCommand: () => ({
        argv: [],
        action: "list_events",
      ...
      transformPayload: () => ({ sanitized: true }),
      postPolicy: ({ payload }) => ({
        allowed: payload.sanitized === true && payload.raw !== true,
      }),
    });
    ...
    expect(result.data.payload).toEqual({ sanitized: true });
  });
});
