...
>>>>>>> e54ddf616 (fix: add changelog for #19077 (thanks @ayanesakura))
...
>>>>>>> ccb415b69 (fix: align ACP permission docs defaults (#31044) (thanks @barronlroth))
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `OPENCLAW_BROWSER_NO_SANDBOX` / `OPENCLAW_BROWSER_NO_SANDBOX`, and add security-audit checks for stale/missing sandbox browser Docker hash labels. Thanks @TerminalsandCoffee and @vincentkoc.
...
- Config/State: removed legacy `.openclaw` auto-detection/migration and `openclaw.json` config candidates. If you still have state/config under `~/.openclaw`, move it to `~/.openclaw` (recommended) or set `OPENCLAW_STATE_DIR` / `OPENCLAW_CONFIG_PATH` explicitly.
