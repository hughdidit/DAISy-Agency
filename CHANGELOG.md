...
>>>>>>> d446722f2 (docs(changelog): merge post-v2026.2.26 release notes)
...
=======
- Tools/Diffs: add a new optional `diffs` plugin tool for read-only diff rendering from before/after text or unified patches, with gateway viewer URLs for canvas and PNG image output. Thanks @gumadeiras.
>>>>>>> fd341d0d3 (docs(changelog): add diffs entry)
- Android/Gateway capability refresh: add live Android capability integration coverage and node canvas capability refresh wiring, plus runtime hardening for A2UI readiness retries, scoped canvas URL normalization, debug diagnostics JSON, and JavaScript MIME delivery. (#28388) Thanks @obviyus.
- Feishu/Doc permissions: support optional owner permission grant fields on `feishu_doc` create and report permission metadata only when the grant call succeeds, with regression coverage for success/failure/omitted-owner paths. (#28295) Thanks @zhoulongchao77.
- Feishu/Docx tables + uploads: add `feishu_doc` actions for Docx table creation/cell writing (`create_table`, `write_table_cells`, `create_table_with_values`) and image/file uploads (`upload_image`, `upload_file`) with stricter create/upload error handling for missing `document_id` and placeholder cleanup failures. (#20304) Thanks @xuhao1.
- Feishu/Reactions: add inbound `im.message.reaction.created_v1` handling, route verified reactions through synthetic inbound turns, and harden verification with timeout + fail-closed filtering so non-bot or unverified reactions are dropped. (#16716) Thanks @schumilin.
...
>>>>>>> 5850045df (fix(cron): land #31145 explicit delivery none in editor (@byungsker))
...
>>>>>>> 4320cde91 (fix(slack): land #31028 from @taw0002)
...
>>>>>>> ee96e1751 (docs(changelog): add missing contributor credits for 2026.3.1 (#31079))
...
>>>>>>> 53a2e72fc (feat(feishu): extract embedded video/media from post (rich text) messages (#21786))
...
- Slack/Native commands: register Slack native status as `/agentstatus` (Slack-reserved `/status`) so manifest slash command registration stays valid while text `/status` still works. Landed from contributor PR #29032 by @maloqab. Thanks @maloqab.
>>>>>>> 8090cb4c5 (docs: missing changelog itesm (#29281))
...
>>>>>>> cdbed3c9b (fix(googlechat): land #30965 thread reply option support (@novan))
...
>>>>>>> 0a182bb4d (docs(changelog): add entries for recent landed Discord PRs)
...
>>>>>>> f53ea0b74 (docs(changelog): add entries for PRs #31090 #31105 #31093 #31088)
...
>>>>>>> 8e6b3ade3 (docs(changelog): record session lifecycle and diagnostics fixes)
...
>>>>>>> e1a9ba840 (docs(changelog): credit GHSA-6f6j reporter)
...
>>>>>>> 51bccaf98 (chore(changelog): note internal events and ingress hardening)
...
>>>>>>> 4b027927c (Changelog: credit startup performance reports)
...
>>>>>>> b9e07ad7b (docs(slack): add missing DM scopes to manifest (openclaw#29999) thanks @JcMinarro)
...
>>>>>>> 635c78a17 (docs: add changelog entry for session_status levels (#30129))
...
### Breaking
=======
>>>>>>> 45b5c2382 (docs(changelog): reorder unreleased changes by user interest)
...
=======
>>>>>>> d33db186d (docs: reorder unreleased 2026.2.26 changelog entries)
...
>>>>>>> 00e8e88a7 (docs(changelog): note auth-profile alias normalization (#26950) (thanks @byungsker))
...
- Typing/TTL safety net: add max-duration guardrails to shared typing callbacks so stuck lifecycle edges auto-stop typing indicators even when explicit idle/cleanup signals are missed. (#27428) Thanks @Crpdim.
>>>>>>> aa17bdbe4 (docs(changelog): reorder all unreleased entries by user impact)
...
<<<<<<< HEAD
- Auto-reply/Streaming: suppress only exact `NO_REPLY` final replies while still filtering streaming partial sentinel fragments (`NO_`, `NO_RE`, `HEARTBEAT_...`) so substantive replies ending with `NO_REPLY` are delivered and partial silent tokens do not leak during streaming. (#19576) Thanks @aldoeliacim.
>>>>>>> 2ca2d5ab1 (docs: add changelog note for sandbox alias fix)
- Doctor/State integrity: ignore metadata-only slash routing sessions when checking recent missing transcripts so `openclaw doctor` no longer reports false-positive transcript-missing warnings for `*:slash:*` keys. (#27375) thanks @gumadeiras.
- Channels/Multi-account config: when adding a non-default channel account to a single-account top-level channel setup, move existing account-scoped top-level single-account values into `channels.<channel>.accounts.default` before writing the new account so the original account keeps working without duplicated account values at channel root; `openclaw doctor --fix` now repairs previously mixed channel account shapes the same way. (#27334) thanks @gumadeiras.
- Telegram/Inline buttons: allow callback-query button handling in groups (including `/models` follow-up buttons) when group policy authorizes the sender, by removing the redundant callback allowlist gate that blocked open-policy groups. (#27343) Thanks @GodsBoy.
- Telegram/Streaming preview: when finalizing without an existing preview message, prime pending preview text with final answer before stop-flush so users do not briefly see stale 1-2 word fragments (for example `no` before `no problem`). (#27449) Thanks @emanuelst for the original fix direction in #19673.
...
>>>>>>> a9d9a968e (chore(changelog): move post release entries to unreleased section)
...
>>>>>>> f60d9591e (docs(changelog): add macOS auth fix note for setup-token path)
...
>>>>>>> f83719937 (Changelog: note Discord embed fallback coverage)
=======
...
- Branding/Docs + Apple surfaces: replace remaining `ai.openclaw` launchd label, bundle-id, logging subsystem, and command examples with `ai.openclaw` across docs, iOS app surfaces, helper scripts, and CLI test fixtures.
...
- Security/Nextcloud Talk: drop replayed signed webhook events with persistent per-account replay dedupe across restarts, and reject unexpected webhook backend origins when account base URL is configured. Thanks @aristorechina for reporting.
>>>>>>> fdea7415c (docs: reorder unreleased changelog by user impact)
...
>>>>>>> 56b8c6948 (docs(changelog): add discord typing fix entry (#26295) (thanks @ngutman))
## 2026.2.24
...
=======
=======
>>>>>>> 2d1e6931a (docs(changelog): reorder and backfill 2026.2.24 release notes)
...
>>>>>>> de586373e (Changelog: note exact do not do that stop trigger)
...
- Auto-reply/Abort shortcuts: expand standalone stop phrases (`stop openclaw`, `stop action`, `stop run`, `stop agent`, `please stop`, and related variants) and accept trailing punctuation (for example `STOP OPENCLAW!!!`) so emergency stop messages are caught more reliably.
>>>>>>> 113545f00 (docs(changelog): note browser control startup import fix (#23974) (thanks @ieaves))
...
>>>>>>> 16b228e4a (fix(macos): resolve webchat panel corner clipping (#22458))
=======
...
>>>>>>> 1970a1e9e (fix(macos): keep Return for IME marked text commit (#25178))
...
>>>>>>> 11a0495d5 (fix(macos): default voice wake forwarding to webchat (#25440))
...
>>>>>>> 31e6d1853 (fix(macos): prefer openclaw binary while keeping pnpm fallback (#25512))
- macOS/Voice input: guard all audio-input startup paths against missing default microphones (Voice Wake, Talk Mode, Push-to-Talk, mic-level monitor, tester) to avoid launch/runtime crashes on mic-less Macs and fail gracefully until input becomes available. (#25817) Thanks @sfo2001.
>>>>>>> 236b22b6a (fix(macos): guard voice audio paths with no input device (#25817))
...
>>>>>>> 8680240f7 (docs(changelog): backfill landed fix PR entries)
...
>>>>>>> 7655c0cb3 (docs(changelog): add synology-chat allowlist fail-closed note)
...
>>>>>>> b0f392580 (docs(changelog): remove next-release shipping sentence)
- Telegram/Media fetch: prioritize IPv4 before IPv6 in SSRF pinned DNS address ordering so media downloads still work on hosts with broken IPv6 routing. (#24295, #23975) Thanks @Glucksberg.
>>>>>>> 9ccc15f3a (docs: update changelog note for native image workspace fix)
...
=======
- Control UI/Chat images: route image-click opens through a shared safe-open helper (allowing only safe URL schemes) and open new tabs with opener isolation to block tabnabbing. (#18685, #25444, #25847) Thanks @Mariana-Codebase and @shakkernerd.
>>>>>>> 853f75592 (changelog: include #25847 in chat image safety entry (#25847) (thanks @shakkernerd))
...
>>>>>>> 46dee2660 (docs(reference): add prompt-caching guide and knobs)
### Breaking
...
- Tools/web_search: add `provider: "kimi"` (Moonshot) support with key/config schema wiring and a corrected two-step `$web_search` tool flow that echoes tool results before final synthesis, including citation extraction from search results. (#16616, #18822) Thanks @adshine.
- Media understanding/Video: add a native Moonshot video provider and include Moonshot in auto video key detection, plus refactor video execution to honor `entry/config/provider` baseUrl+header precedence (matching audio behavior). (#12063) Thanks @xiaoyaner0201.
>>>>>>> 4c21ef9ce (docs(changelog): correct kimi issue references)
...
>>>>>>> 65d57eac1 (docs(changelog): reorder 2026.2.23 entries by user impact)
...
>>>>>>> 5e1dd5fe6 (Changelog: add PR #24593 entry)
...
>>>>>>> b19a6ee62 (docs(changelog): move mistral to top and add synology chat)
...
>>>>>>> 1c2c7843a (docs: add synology channel docs and fix unreleased changelog)
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `OPENCLAW_BROWSER_NO_SANDBOX` / `OPENCLAW_BROWSER_NO_SANDBOX`, and harden the default container security posture (`GHSA-43x4-g22p-3hrq`). Thanks @TerminalsandCoffee and @vincentkoc.
=======
=======
...
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and OpenClaw targeting work. (#2013) Thanks @alexgleason.
...
- eef247b7 fix: auto-inject Telegram forum topic threadId in message tool (OpenClaw).
- 6ac5dd2c test: cover telegram topic threadId auto-injection and subagent origin threading (OpenClaw).
- a13efbe2 fix: pass threadId/to/accountId from parent to subagent gateway call (OpenClaw).
...
- 5f6e1c19 feat(feishu): sync with openclaw-feishu #137 (multi-account support) (Yifeng Wang).
...
- Docs: fix typo - openclaw is the compatibility shim, not openclaw. (#7415) Thanks @lailoo.
...
### Highlights
- Rebrand: rename the npm package/CLI to `openclaw`, keep a `openclaw` compatibility shim, move extensions to the `@openclaw/*` scope, and update ai.openclaw bundle IDs/labels/logging subsystems. Thanks @thewilloftheshadow.
...
- Branding: update launchd labels, mobile bundle IDs, and logging subsystems to ai.openclaw (legacy com.openclaw migrations). Thanks @thewilloftheshadow.
...
- CLI: add `openclaw system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.molt.bot/cli/system
- CLI: add live auth probes to `openclaw models status` for per-profile verification. (commit 40181afde) https://docs.molt.bot/cli/models
...
- Doctor: honor OPENCLAW_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- CLI: add `openclaw update wizard` for interactive channel selection and restart prompts. https://docs.molt.bot/cli/update
...
- ACP: add `openclaw acp` for IDE integrations. https://docs.molt.bot/cli/acp
- ACP: add `openclaw acp client` interactive harness for debugging. https://docs.molt.bot/cli/acp
...
- Exec approvals: migrate approvals to `~/.openclaw/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.molt.bot/tools/exec-approvals
- Nodes: add headless node host (`openclaw node start`) for `system.run`/`system.which`. https://docs.molt.bot/cli/node
...
### Fixes
- Discovery: shorten Bonjour DNS-SD service type to `_openclaw-gw._tcp` and update discovery clients/docs.
...
- Plugins: add Zalo Personal plugin (`@openclaw/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.molt.bot/plugins/zalouser
...
- **BREAKING:** `openclaw hooks` is now `openclaw webhooks`; hooks live under `openclaw hooks`. https://docs.molt.bot/cli/webhooks
...
- Daemon: share profile/state-dir resolution across service helpers and honor `OPENCLAW_STATE_DIR` for Windows task scripts.
...
- Onboarding/Auth: honor `OPENCLAW_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `OPENCLAW_LAUNCHD_LABEL` / `OPENCLAW_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `openclaw doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- macOS: prompt to install the global `openclaw` CLI when missing in local mode; install via `molt.bot/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer UX: support `OPENCLAW_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Config: expand `~` in `OPENCLAW_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: respect `OPENCLAW_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`OPENCLAW_LIVE_GATEWAY_IMAGE_PROBE=1`).
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle OpenClawKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
