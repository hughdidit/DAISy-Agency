...
>>>>>>> b9aa2d436 (fix(security): enforce sandbox inheritance for sessions_spawn)
...
>>>>>>> a0d2f6e4f (docs(changelog): note skills security hardening)
...
- Microsoft Teams/File uploads: acknowledge `fileConsent/invoke` immediately (`invokeResponse` before upload + file card send) so Teams no longer shows false "Something went wrong" timeout banners while upload completion continues asynchronously; includes updated async regression coverage. Landed from contributor PR #27641 by @scz2011.
>>>>>>> b678308d9 (docs: add unreleased security note for msteams ssrf hardening)
...
>>>>>>> 0a23739c3 (fix(feishu): pass proxy agent to WSClient for proxy environments (#26397))
...
>>>>>>> b564b72dc (docs(changelog): add missing security PR entries (#26118 #26116 #26112 #26111 #26095))
...
>>>>>>> 8470dff61 (chore(deps): update dependencies except carbon)
### Breaking
...
>>>>>>> f8524ec77 (fix(security): harden exported session html rendering)
...
>>>>>>> 2e36bdda8 (docs(changelog): credit ACP security reporter)
...
>>>>>>> 02772b029 (fix(security): require sender-only matching for elevated allowFrom)
- Security/Feishu: enforce ID-only allowlist matching for DM/group sender authorization, normalize Feishu ID prefixes during checks, and ignore mutable display names so display-name collisions cannot satisfy allowlist entries. This ships in the next npm release. Thanks @jiseoung for reporting.
- Security/Group policy: harden `channels.*.groups.*.toolsBySender` matching by requiring explicit sender-key types (`id:`, `e164:`, `username:`, `name:`), preventing cross-identifier collisions across mutable/display-name fields while keeping legacy untyped keys on a deprecated ID-only path. This ships in the next npm release. Thanks @jiseoung for reporting.
...
<<<<<<< HEAD
>>>>>>> de2e5c7b7 (docs(security): clarify dangerous control-ui bypass policy)
=======
...
>>>>>>> 498138e77 (docs(changelog): record avatar security hardening)
